...
data {
  ...
  array[4] real y0;               // Initial state including total population (SIRN)
...
model {
  ...
  // t0 = 0
  array[T, 3] real sir_sol = integrate_ode_rk45(sir, y0, 0, ts, theta, x_r, x_i, 1e-6, 1e-5, 100000);

  // Binomial likelihood for the observed infections
  for (i in 1:T) {
    // sample Bin(1000, I / N)
    sir_sol[i, 3] = fmax(sir_sol[i, 3], 0.0);  // deal with underflow
    infections[i] ~ binomial(total_count, sir_sol[i, 3] / y0[4]);
  }
}
generated quantities {
  real log_beta_prior = normal_rng(log(0.4), 0.5);
  real log_gamma_prior = normal_rng(log(1.0/8.0), 0.2);
}